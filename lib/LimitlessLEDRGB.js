 ...
 */
function LimitlessLEDRGB(ipAddress) {
...
LimitlessLEDRGB.prototype.write = function(data) {
  ...
  if (data == BLACK) {
    functionToCall = function () {
      setRGBLED(0, this.ipAddress);
    }.bind(this);
  }
  else if (data == WHITE) {
    functionToCall = function () {
      setRGBMode(MODEPREV, this.ipAddress);
    }.bind(this);
    turnOnFirst = true;
  }
    ...
    if (colourIndex != -1) { //is a colour
      functionToCall = function() {
        setRGBColour(lllColourLookup[colourIndex], this.ipAddress);
      }.bind(this);
    turnOnFirst = true;
    }
  }
  if (typeof functionToCall !== 'undefined') {
    if (turnOnFirst) {
      setRGBLED(1, this.ipAddress, functionToCall);
    }
    else {
      functionToCall();
    }
...
function setRGBLED(on, ipAddress, callback) {

  var cmdByte = (on ? cmdOnByte : cmdOffByte);
  sendUDPCommand(cmdByte, valByteNA, ipAddress, callback);
}

const MODENEXT = true;
const MODEPREV = false;
function setRGBMode(nextMode, ipAddress) {
  var cmdByte = (nextMode ? cmdModeNextByte : cmdModePrevByte);
  sendUDPCommand(cmdByte, valByteNA, ipAddress);
}

function setRGBColour(valueByte, ipAddress) {

  sendUDPCommand(cmdColourByte, valueByte, ipAddress);
}
...
function sendUDPCommand(cmdByte, valByte, ipAddress, callback) {
  ...
  client.send(cmd, 0, cmd.length, 50000, ipAddress, didFinishSend)
}
